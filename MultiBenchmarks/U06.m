function [InitFunction, CostFunction, FeasibleFunction] = U06
...
for pop = 1 : num
    ...
    J1 = 3 : 2 : dim;
    Population(pop).cost(1) = x(1) + max(0,(2*(((1/2*N) + epsi)*abs(sin(2*N*pi*x(1)))))) + ((2 / length(J1))* ((4*temp1) - (2*temp2) + 2));
    % f2
    temp3 = 0;
    for j = 2 : 2 : dim
        temp3 = temp3 + y(j)^2;
    ...
    J2 = 2 : 2 : dim;
    Population(pop).cost(2) = 1 - x(1) + max(0,(2*(((1/2*N) + epsi)*abs(sin(2*N*pi*x(1)))))) + ((2 / length(J2))* ((4*temp3) - (2*temp4) + 2));
end
return