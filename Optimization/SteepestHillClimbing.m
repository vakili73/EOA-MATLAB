function [MinCost] = SteepestHillClimbing(ProblemFunction, DisplayFlag)
% Steepest ascent hill climbing algorithm for optimizing a general function.
...
while feval <= OPTIONS.MaxFEval
    features = OPTIONS.MinDomain + (OPTIONS.MaxDomain - OPTIONS.MinDomain) .* rand(1, OPTIONS.numVar);
    for q = 1 : OPTIONS.numVar
        Children(q).chrom = Population(1).chrom;
        Children(q).chrom(q) = features(q);
    end
    Children = OPTIONS.CostFunction(Children, OPTIONS);
    feval = feval + OPTIONS.numVar;
    [~, BestIndex] = sort([Children.cost], 'ascend');
    if Population(1).cost < Children(BestIndex(1)).cost
        Population = OPTIONS.InitFunction(OPTIONS);
        Population = OPTIONS.CostFunction(Population, OPTIONS);
        feval = feval + 1;
    else
        Population(1) = Children(BestIndex(1));
    end
